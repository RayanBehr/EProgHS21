	...
	public static boolean task2(Room room) {			
		...
		for(Room ajRoomA : room.doorsTo) for(Room ajRoomB : room.doorsTo)							
			if(ajRoomA.getAge() == ajRoomB.getAge()) {
				...
				if(rec2(ajRoomA, ajRoomB, copy(memoA), copy(memoB))) return true;	
				memoA.pop(); memoB.pop(); }	
		return false;
	}
	
	public static boolean rec2(Room roomA, Room roomB, Stack<Room> memoA, Stack<Room> memoB) {
		for(Room ajRoomA : roomA.doorsTo) for(Room ajRoomB : roomB.doorsTo)			
			if(ajRoomA.getAge() == ajRoomB.getAge()) {			
				memoA.push(ajRoomA); memoB.push(ajRoomB);
				if(rec2(ajRoomA, ajRoomB, copy(memoA), copy(memoB))) return true;			
				memoA.pop(); memoB.pop(); }		
		if(roomA.isExit() && roomB.isExit())
			while(memoA.size() > 0) if(memoA.pop() != memoB.pop()) return true;				
		return false;
	}
	
	public static Stack<Room> copy(Stack<Room> st) {	
		Stack<Room> flip = new Stack<>();
		while(st.size() > 0) flip.push(st.pop());		
		Stack<Room> stCopy = new Stack<>();
		while(flip.size() > 0) {
			Room ele = flip.pop();
			stCopy.push(ele); st.push(ele); }	
		return stCopy;	
	}
